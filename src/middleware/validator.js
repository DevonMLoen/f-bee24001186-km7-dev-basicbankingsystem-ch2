...
const userSchema = joi.object({
  ...
  userPassword: joi.string().min(10).required().messages({
  ...
  profileType: joi.string().required().messages({
    "string.base": "Profile type must be a string.",
    "any.required": "Profile type is required.",
  }),

  profileNumber: joi.string().required().messages({
...
const bankAccountSchema = joi.object({
  ...
  bankAccountNumber: joi.string().min(6).max(20).required().messages({
...
const transactionSchema = joi.object({
  sourceAccountId: joi.number().integer().positive().required().messages({
  ...
  destinationAccountId: joi.number().integer().positive().required().messages({
...
const resetPasswordSchema = joi.object({
  ...
  confirmPassword: joi.string().min(10).required().messages({
    "string.base": "New password must be a string.",
    "string.min": "New password must have at least 10 characters.",
